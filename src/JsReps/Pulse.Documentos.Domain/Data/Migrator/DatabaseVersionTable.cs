...
namespace Pulse.Documentos.Domain.Data.Migrator {
    internal class DatabaseVersionTable {
        ...
        private string TableName =>
            (HasSchema ? $"{SchemaName}." : string.Empty) + DatabaseVersionTableName;
        ...
        public bool Exists(MigratorDatabase migratorDatabase) {
            return 0 != migratorDatabase.SqlQuery<int>($@"SELECT count(*) FROM INFORMATION_SCHEMA.TABLES  WHERE TABLE_NAME = N'DatabaseVersion'").Single();
        }
